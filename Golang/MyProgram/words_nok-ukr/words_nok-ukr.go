...
func main() {
	...
	defer logger.Sync()
	sugar := logger.Sugar()
	sugar.Infow("Started words_nok-ukr: localhost:8080/word")

	//  открываем файл
	jsonFile, err := os.Open("myWords.json")
	if err != nil {
...
func wordAdd(w http.ResponseWriter, r *http.Request) {
	...
	if r.FormValue("WordOriginal") != "" {
		...
		if !wordExists {
			...
			jsonFile, err := os.OpenFile("myWords.json", os.O_WRONLY|os.O_TRUNC, 0644)
			if err != nil {
