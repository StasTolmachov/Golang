...
func wordAll(w http.ResponseWriter, r *http.Request) {
	...
	defer logger.Sync()
	sugar := logger.Sugar()
	sugar.Infow("Started wordAll")

	//  открываем файл
	sugar.Infow("открываем файл: EnglishForEveryone.json")
	jsonFile, err := os.Open("EnglishForEveryone.json")
	
	if err != nil {
		sugar.Infow("Ошибка открытия файла: EnglishForEveryone", err)
		return
	}
	defer jsonFile.Close()

	// Читаем содержимое файла
	sugar.Infow("Читаем файл: EnglishForEveryone.json")
	jsonData, err := ioutil.ReadAll(jsonFile)
	...
	// Десериализуем JSON в структуру
	sugar.Infow("Десериализуем файл: EnglishForEveryone.json")
	err = json.Unmarshal(jsonData, &Words)
